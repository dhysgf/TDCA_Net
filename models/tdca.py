...
from models.base import FilterBankSSVEP
...
def tdca_feature(
    ...
    rhos = []
    fiter_data=[]#空间滤波后的数据集

    for Xk, P in zip(templates, Ps):#40*12*250
        a = xiang_dsp_feature(
            W,
            M,
    ...
    return np.array(rhos),np.array(fiter_data)
...
class TDCA(BaseEstimator, TransformerMixin, ClassifierMixin):
    ...
    def predict(self, X: ndarray):
        feat,fiter_data = self.transform(X)
...
class FBTDCA(FilterBankSSVEP, ClassifierMixin):
    ...
    def fit(self, X: ndarray, y: ndarray, Yf: Optional[ndarray] = None):  # type: ignore[override]
        ...
        super().fit(X, y, Yf=Yf)
        return self
    def transform(self, X: ndarray):  # typ
        ros,features = self.transform(X)
